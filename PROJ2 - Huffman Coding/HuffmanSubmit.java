...
public class HuffmanSubmit implements Huffman {
	public static class Node {
		...
		public Node(Character c, Integer freq, Node left, Node right) { // constructor node class
			this.c = c;
			this.freq = freq;
			this.left = left;
			this.right = right;
		...
		public Boolean isLeaf() { // check if leaf node
			return (this.left == null && this.right == null);
		}
	}

	public static void getCodes(String[] table, Node node, String prefix) { // recursive helper to get codes
		if (node.isLeaf()) {
	...
	public static int[] getCharFreq(String inputString) {
		...
		while (!in.isEmpty()) { // count frequencies
			char c = in.readChar();
			freq[c]++;
		}
		return freq;
	...
	public static Node buildTree(int[] freq) {
		...
		for (char i = 0; i < freq.length; i++) { // create leaf nodes
			if (freq[i] > 0) {
				pQueue.add(new Node(i, freq[i], null, null));
			}
		}

		while (pQueue.size() > 1) { // build tree
 ...
 *                     */
	...
	public void encode(String inputFile, String outputFile, String freqFile){
		...
		try (FileWriter fWriter = new FileWriter(freqFile)) {
			for (int i = 0; i < freq.length; i++) { // create freq file
		...
		while (!input.isEmpty()) {
			...
			String code = codes[c];
			for (int i = 0; i < code.length(); i++) { // write encoded bits
				if (code.charAt(i) == '1') {
					output.write(true);
				} else {
					output.write(false);
 ...
 *                     */
	...
	public void decode(String inputFile, String outputFile, String freqFile){
		...
		try (BufferedReader bReader = new BufferedReader(new FileReader(freqFile))) {
			String line;
			while ((line = bReader.readLine()) != null) { // read freq file
		...
		BinaryOut output = new BinaryOut(outputFile);
		Node curr = root;
		while (!input.isEmpty()) { // traverse tree according to bits
			...
			if (curr.isLeaf()) { // if leaf node, write char to output
				output.write(curr.c);
				curr = root;
			}
		}
