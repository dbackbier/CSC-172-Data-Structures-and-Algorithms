    ...
    public static int count(int[] a) {
        ...
        for (int i = 0; i < n; i++) { // loops n times, O(n), and O(1) assignment operation inside
            for (int j = i+1; j < n; j++) { // loops n times, O(n), and O(1) assignment operation inside
                for (int k = j+1; k < n; k++) { // loops n times, O(n), and O(1) assignment operation inside
                    if (a[i] + a[j] + a[k] == 0) { // O(1)
                        count++; // O(1), only executed when condition is true
                    }
                }
            }
        }
        return count; // O(1)
    } // O(n) * O(n) * O(n) = O(n^3), constant time operations can be ignored for Big-Oh notation
