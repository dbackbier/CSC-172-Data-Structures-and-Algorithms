...
public class TwoSumFast {
	...
	public static int count(int[] a) {
		...
		for (int i = 0; i < n; i++) { // loops n times, O(n), and O(1) assignment operation inside
			...
			if (j > i) count++; // O(1) (only executed when condition is true)
		}
		return count; // O(1)
	} // O(n) * O(log n) = O(n log n), constant time operations can be ignored for Big-Oh notation

	// returns true if the sorted array a[] contains any duplicated integers
	private static boolean containsDuplicates(int[] a) { // O(n)
