...
public class ThreeSumFast {
    ...
    public static int count(int[] a) {
        ...
        if (containsDuplicates(a)) throw new IllegalArgumentException("array contains duplicate integers"); // O(n), from containsDuplicates call
        int count = 0; // O(1), assignment
        for (int i = 0; i < n; i++) { // loops n times, O(n), and O(1) assignment operation inside
            for (int j = i+1; j < n; j++) { // loops n times, O(n), and O(1) assignment operation inside
                int k = Arrays.binarySearch(a, -(a[i] + a[j])); // O(log n)
                if (k > j) count++; // O(1) (only executed when condition is true)
            }
        }
        return count; // O(1)
    } // O(n) * O(n) * O(log n) = O(n^2 log n), constant time operations can be ignored for Big-Oh notation
